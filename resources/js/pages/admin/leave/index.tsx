...
import * as leaveRoutes from '@/routes/admin/leave';
...
export default function AdminLeaveIndex({ leaves: data, filters }: Props) {
    ...
    const approve = (id: number) => {
        router.post(leaveRoutes.approve({ leave: id }).url);
    };
        ...
        <>
            ...
            <div className="flex flex-col gap-6 p-6">
                <div>
                    ...
                    <p className="text-muted-foreground text-sm">{data.total} total requests</p>
                </div>
                                    ...
                                    {data.data.map((leave) => (
                                        <tr key={leave.id} className="border-b last:border-0 hover:bg-muted/20">
                                            <td className="px-4 py-3">
                                                ...
                                                <p className="text-muted-foreground text-xs">{leave.employee?.employee_id}</p>
                                            </td>
                                            ...
                                            <td className="px-4 py-3">
                                                {leave.status === 'pending' && (
                                                    <div className="flex gap-1">
                                                        ...
                                                        <Button size="sm" variant="outline" className="text-red-600" onClick={() => openReject(leave.id)}>
                                                            <X className="h-3 w-3" />
                                                        </Button>
                                                    </div>
                                                )}
                ...
                <DialogContent>
                    ...
                    <div className="space-y-4">
                        <div className="space-y-1">
                            <Label>Reason *</Label>
                            <Input value={reason} onChange={(e) => setReason(e.target.value)} placeholder="State the reason…" />
                        </div>
