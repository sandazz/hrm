...
interface Payroll {
    ...
    base_salary: number; gross_salary: number;
    overtime_pay: number; bonus: number; allowances: number;
    deductions: number; no_pay_deduction: number; late_deduction: number;
    epf_employee: number; epf_employer: number; etf_employer: number;
    tax: number; net_salary: number;
    working_days: number; present_days: number; leave_days: number;
    overtime_hours: number; late_days: number; no_pay_days: number;
    status: string; paid_at?: string; notes?: string;
    employee?: {
                ...
                <Card className="mx-auto max-w-3xl print:shadow-none">
                    <CardHeader className="border-b">
                        <div className="flex items-start justify-between">
                            ...
                            <Badge variant={payroll.status === 'paid' ? 'default' : payroll.status === 'processed' ? 'secondary' : 'outline'} className="text-sm">
                    ...
                    <CardContent className="space-y-6 pt-6">
                        ...
                        <div className="grid grid-cols-2 gap-4 rounded-lg bg-muted/30 p-4 text-sm">
                            ...
                            <div>
                                ...
                                {payroll.late_days > 0 && <p className="text-orange-600 text-xs">{payroll.late_days} late days</p>}
                            </div>
                            <div>
                                <p className="text-muted-foreground text-xs">Period</p>
                                <p>{MONTHS[payroll.month - 1]} {payroll.year}</p>
                                {payroll.paid_at && <p className="text-xs text-green-600">Paid: {new Date(payroll.paid_at).toLocaleDateString()}</p>}
                            </div>
                        </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex justify-between font-semibold">
                                        ...
                                        <span className="text-red-600">
                                            ({deductionRows.reduce((s, r) => s + r.amount, 0).toLocaleString()})
                                        </span>
                                    </div>
                                </div>
                            </div>
